...
# Read Posts from Server by Category
@app.route("/posts/category/<string:category>")
def readPostsByCategory(category):
    limit = request.args.get("limit", 15)
    page = request.args.get("page", 1)

    filterQuery = {"tags": {"$elemMatch": {"$regex": category, "$options": "i"}}}
